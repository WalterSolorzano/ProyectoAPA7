...
def _iter_all(storage_dir: Path) -> dict[str, tuple[PresetRecord, Path, str]]:
    ...
    for path in sorted(BUILTIN_DIR.glob("*.json")):
        rec = _read_record(path, "builtin")
        out[rec.name] = (rec, path, "builtin")
    for path in sorted(_user_dir(storage_dir).glob("*.json")):
        rec = _read_record(path, "user")
        out[rec.name] = (rec, path, "user")
    return out
